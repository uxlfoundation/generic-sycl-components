...
#ifndef ONEMATH_SYCL_BLAS_BLAS3_NO_LOCAL_FULL_VEC_GEMM_HPP
#define ONEMATH_SYCL_BLAS_BLAS3_NO_LOCAL_FULL_VEC_GEMM_HPP
 ...
 public:
  ...
  ONEMATH_SYCL_BLAS_INLINE void eval(sycl::nd_item<1> id) noexcept {
    ...
    const auto boundary_check_original_m =
        [&](const index_t &idx) ONEMATH_SYCL_BLAS_ALWAYS_INLINE {
          return local_item_id_row + wg_row + idx < original_m;
        };
    const auto boundary_check_n =
        [&](const index_t &idx) ONEMATH_SYCL_BLAS_ALWAYS_INLINE { return idx < n; };
...
#pragma unroll
 ...
 private:
  ...
  ONEMATH_SYCL_BLAS_INLINE void load_and_compute_block(
    ...
    load_block_a<item_rows, packet_size, wg_rows * packet_size, check_boundary,
        ...
        [=](const index_t &idx) ONEMATH_SYCL_BLAS_ALWAYS_INLINE { return idx < k; },
        out_of_range);
...
#pragma unroll
  ...
  ONEMATH_SYCL_BLAS_INLINE typename std::enable_if<!trans>::type load_block_a(
...
#pragma unroll
  ...
  template <bool check_row, bool check_col, index_t work_per_load, bool trans,
            typename PointerType, typename RowCheckType, typename ColCheckType>
  ONEMATH_SYCL_BLAS_INLINE typename std::enable_if<!trans>::type load_single_b(
...
#pragma unroll
  ...
  ONEMATH_SYCL_BLAS_INLINE typename std::enable_if<!trans>::type
...
#pragma unroll
   ...
   */
  template <index_t packet_size, bool trans = trans_b>
  ONEMATH_SYCL_BLAS_INLINE typename std::enable_if<(packet_size != 1 && trans)>::type
  compute_block_gemm_no_shared(index_t iteration, value_t *reg_a,
                               value_t *reg_b, element_t *reg_res) noexcept {
...
#endif  // ONEMATH_SYCL_BLAS_BLAS3_NO_LOCAL_FULL_VEC_GEMM_HPP
